...
$aidocDB = "aidocapp"
$AidocUser = "aidocapp"
$key = Get-Content "$($Scriptpath)\Secret\key.key"
$Key -join ',' | Out-Null
$secure = Get-Content "$($Scriptpath)\Secret\Encrypted.txt" | ConvertTo-SecureString -key $Key
...
$Versions = ($PostgreSQLWebData | Where-Object { $_.field_os -eq $bit }).PostgreSQLVersion
